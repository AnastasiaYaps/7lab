#include <iostream>
	...
	public:
		...
		T& operator*() {
			if (node == nullptr) throw Exception("Empty iterator");
			return node->value;
		}
		...
		Iterator operator++(int unused) { 
			if (node == nullptr)
				throw Exception("Empty iterator");
			Iterator res = *this; node = node->right;
			return res; }
		Iterator operator--(int unused) { 
			if (node == nullptr) 
				throw Exception("Empty iterator"); 
			Iterator res = *this; 
			node = node->left; return res; }
	...
	int main() {
		...
		cout << "\nInserting\n";
		try {
			stac.push(Game("A", 213, 10));
			stac.show();
			stac.push(Game("D", 454, 45));
			stac.show();
			stac.push(Game("B", 645, 6));
			stac.show();
			stac.push(Game("K", 9841, 7));
			stac.show();
			stac.push(Game("M", 123, 9));
			stac.show();
			stac.push(Game("X", 784, 4));
			stac.show();
		}
		...
		try {
			...
			try {
				cout << "\nErase K\n";
				stac.erase(stac.find(Game("K", 9841, 7)));
			}
			catch (Exception ex) { cerr << ex; };
			stac.show();
			try {
				auto dan = stac.find(Game("D", 454, 45));
				auto anz = stac.find(Game("B", 645, 6));
				stac.swap(dan, anz);
			}
			catch (Exception ex) { cerr << ex; };
			stac.show();
			try {
				Stack<Game>::Iterator bgn = stac.begin();
				Stack<Game>::Iterator end = stac.begin();
